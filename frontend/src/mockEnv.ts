...
// Mock the environment when not in Telegram (works in both dev and production builds)
// Check if we're actually in Telegram, and if not, always mock
const themeParams = {
...
const setupMock = () => {
  ...
  console.info(
    "⚠️ Environment mocked for browser testing (not in Telegram). This allows testing outside Telegram.",
  );
};

// Try to check if we're in Telegram, but always mock if check fails or we're not in Telegram
try {
  ...
  // If isTMA check fails (e.g., in regular browser), mock anyway
  console.warn("Telegram environment check failed, mocking anyway for browser testing:", e);
  setupMock();
}
